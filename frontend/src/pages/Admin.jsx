...
const [lastId, setLastId] = useState(0); // ДОБАВЬТЕ ЭТО
...
useEffect(() => { 
  ...
  const stats = useMemo(() => {
    ...
    const sellers = users.filter(u => (u.role || u.Role || '').toLowerCase() === 'seller').length;
    const customers = users.filter(u => (u.role || u.Role || '').toLowerCase() === 'customer').length;

    return {
        total: lastId, // Общее количество из БД
        admins,
        sellers,
        customers
    };
  ...
  const totalPages = useMemo(() => {
    return Math.max(1, Math.ceil(lastId / USERS_PER_PAGE));
  }, [lastId]);
...
<div className="mt-6 flex justify-center items-center gap-2">
  <button 
    ...
    className="px-3 py-1 bg-gray-800 text-slate-300 rounded hover:bg-gray-700 disabled:opacity-30"
  >Назад</button>
